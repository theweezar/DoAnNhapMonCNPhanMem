...
class Friends{
  ...
  getFriends(userID){
    return new Promise((resolve, reject) => {
      ...
      WHERE ${tbName.friends}.userId_1 = ${userID} OR ${tbName.friends}.userId_2 = ${userID} ORDER BY
  ...
  setTimeForLastestMsg(chatID){
    this.conn.query(`UPDATE ${tbName.friends} SET recent = now() WHERE id = ${chatID}`, err => {throw err;});
  }
